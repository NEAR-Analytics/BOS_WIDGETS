...
<html>
  ...
  window.addEventListener("message", function (event) {
    // Check if the event data type is "openLink"
    if (event.data && event.data.type === "openLink") {
      // Open the link provided in the event data
    ...
    function getDetail (row) {
      ...
      if(user.social !== "https://social.near.page/u/"){
       ...
       const HTML = '<div class="popup">'+
        ...
        ? "<a href='" +
          user.social +
          "' target='_blank' onclick='handleLink(" +
          JSON.stringify(user.social) +
          ")'   >  "
        : "") +
...
return (
  <Container>
    ...
    <iframe
      ...
      srcDoc={code}
      onLoad={() => {
        const iframe = document.getElementById("myMap");
        iframe.contentWindow.postMessage({ type: "initialize" }, "*");

        const iframe1 = document.getElementById("myMap");
        iframe1.contentWindow.postMessage(
          { type: "openLink", url: state.url },
          "*"
        );
      }}
    />
