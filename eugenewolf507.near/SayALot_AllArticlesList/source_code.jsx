...
function getLastEditionsByArticle() {
  ...
  let newFormatArticlesData =
    ...
    allArticles
      ...
      .filter((articleIndex) =>
        writersWhiteList.includes(articleIndex.accountId)
      )
      .filter(
        (articleIndex) => !articleBlackList.includes(articleIndex.blockHeight)
      );

  let lastestEditArticlesDataArray =
    newFormatArticlesData &&
    newFormatArticlesData.filter((articleData) => {
  ...
  let finalNewFormatArticles =
    lastestEditArticlesDataArray &&
    lastestEditArticlesDataArray.map((latestEditArticle) => {
      const article = Social.get(
        `${latestEditArticle.accountId}/${addressForArticles}/main`,
        latestEditArticle.blockHeight
      );

      let articleParsed = article && JSON.parse(article);
  ...
  let finalOldFormatArticles =
    oldFormatArticlesArray[0] &&
    finalNewFormatArticles &&
    oldFormatArticlesArray.filter((oldFormatArticle) => {
  ...
  let finalArticles =
    finalNewFormatArticles &&
    finalNewFormatArticles.concat(finalOldFormatArticles);
  ...
  <div className="row card-group py-3">
    {finalArticles.length > 0 &&
      finalArticles.map((article) => {
        ...
        if (!Array.isArray(article.tags) && article) {
          article.tags = [];
        }
        return (
          <div
