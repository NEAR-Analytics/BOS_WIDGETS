...
if (allPost.length > 0) {
  allPost.forEach((item) => {
    ...
    if (allComment7Days.length + allLike7Days.length + allRepost7Days.length > 0) {
      postEngagement.push({
        accountId: item.accountId,
        blockHeight: item.blockHeight,
        allLike: allLike7Days.length,
        allComment: allComment7Days.length,
        allRepost: allRepost7Days.length,
        EP:
          (allComment7Days.length * 3 + allLike7Days.length + allRepost7Days.length * 2) /
          Math.floor(
...
const labelN = "Top trending posts in 7days on NEAR Social";
