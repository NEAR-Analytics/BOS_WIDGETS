...
const getLineProps = (data, category, key, colors, chartOption) => {
  data = data || [];
  ...
  data.forEach((x) => {
    if (categoryKeys.includes(x[category])) {
      return;
    }

    categoryKeys.push(x[category]);
  });

  const dataFormat = categoryKeys.map((categoryKey, index) => {
    const dataFormated = data
      .filter((x) => x[category] == categoryKey)
      .map((d) => [new Date(d["DATE"]).getTime(), d[key]]);
    return {
      data: dataFormated,
      name: categoryKey,
      type: "line",
...
const getAreaProps = (data, category, key, colors, chartOption) => {
  data = data || [];
  colors = colors || themeColors;
  chartOption = chartOption || {};
            ...
            <div className="col-sm-12 col-lg-6">
              <Widget
                ...
                props={getLineProps(
                  ...
                  undefined,
                  {
                    title: "Number of Contract Calls",
                  }
                )}
              />
            </div>
            ...
            <div className="col-sm-12 col-lg-6">
              <Widget
                ...
                props={getAreaProps(
                  ...
                  undefined,
                  {
                    title: "Cumulative Number of Transactions (NEAR)",
                  }
                )}
              />
            </div>
