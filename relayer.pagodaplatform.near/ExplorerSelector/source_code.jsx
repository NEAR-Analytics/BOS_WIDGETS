...
const PikespeakButton = styled.a`
  height: 191px;
  width: 264px;
  border: ${state.selected === "pikespeakai" ? "2px solid #338E7B" : ""};
  position: relative;
  border-radius: 10px;
  box-shadow: ${state.selected === "pikepeakai" ? "" : " 0 0 3px  #999"};
  margin: 10px;
  display: flex;
  justify-content: center;
  align-items: center;
...
return (
  <Main>
    ...
    <Container>
      ...
      <PikespeakButton
        href={
          !hasLinkNearExplorer ? config.pikepeakai + path : config.pikepeakai
        }
        onClick={() => {
          onSelect({ selected: "pikepeakai" });
        }}
      >
        <ImageTab
          src={
