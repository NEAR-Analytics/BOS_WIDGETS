...
const isMatchedAddresses = () => {
  return isLoggedIn() && state.address === walletAddress;
};
...
console.log("relogin err", state.reLoginError);

const onRelogin = () => {
  const res = Ethers.send("wallet_requestPermissions", [{ eth_accounts: {} }]);
  console.log(res);
  if (!!res) {
    State.update({
      reLoginError:
        "Request incompleted! Please refresh the page and try again!",
    });
  }
};
    ...
    <div>
      <div class="alert alert-danger" role="alert">
        {!!state.reLoginError ? (
          ...
          <>
            <h4 class="alert-heading">Address mismatch!</h4>
            <p>
              You are logging in to Impetus Network and BOS using different
              account
            </p>
            <p>
              You Impetus address is: <strong>{walletAddress}</strong>
            </p>
            <p>
              Your BOS address is: <strong>{state.address}</strong>
            </p>
            <p>
              Please try to login to BOS again, using the same account on
              Impetus
            </p>
            <h5>
              Must select <strong>ONLY ONE ACCOUNT</strong>
            </h5>
            <p>
              If you reject the request, please refresh the page and try again.
            </p>
          </>
        )}
      </div>
      <button class="btn btn-primary" onClick={() => onRelogin()}>
        Re-Login
      </button>
    </div>
  );
        ...
        <div class="justify-content-sm-center">
          <div class="input-group mb-3">
            <input
              ...
              onChange={(e) => State.update({ claimAddress: e.target.value })}
            />
            <button
    ...
    <>
      ...
      {isLoggedIn() && (
        <div class="px-4 py-5 my-5 text-center">
          {!isMatchedAddresses() && renderReLogin()}
          {isMatchedAddresses() && renderHero()}
        </div>
      )}
    </>
  );
            ...
            {state.activeLink === "create-giveaway" && (
              <>
                {isLoggedIn() && isMatchedAddresses() ? (
                  ...
                  "Please go back to Dashboard to login"
                )}
              </>
            )}
