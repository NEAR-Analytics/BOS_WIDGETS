...
function createCommunity({ inputs }) {
  return Near.call("devhub.near", "create_community", { inputs });
}
...
function getFeaturedCommunities() {
  return (
    Near.view("devhub.near", "get_featured_communities") ?? null
  );
}
...
function updateCommunityBoard({ handle, board }) {
  return Near.call("devhub.near", "update_community_board", {
    handle,
    board,
  });
}
...
function updateCommunityGithub({ handle, github }) {
  return Near.call("devhub.near", "update_community_github", {
    handle,
    github,
  });
}
...
function getAllAuthors() {
  return Near.view("devhub.near", "get_all_authors") ?? null;
}
...
function getAvailableAddons() {
  ...
  // return Near.view("devhub.near", "get_available_addons") ?? null;
}

function getCommunityAddons({ handle }) {
  return Near.view("devhub.near", "get_community_addons", {
    handle,
  });
}

function getCommunityAddonConfigs({ handle }) {
  return Near.view("devhub.near", "get_community_addon_configs", {
    handle,
  });
}

function getAllLabels() {
  return Near.view("devhub.near", "get_all_labels") ?? null;
}

function getPost({ post_id }) {
  return Near.view("devhub.near", "get_post", { post_id }) ?? null;
}

function getPostsByAuthor({ author }) {
  return (
    Near.view("devhub.near", "get_posts_by_author", { author }) ??
    null
  );
}
...
function getPostsByLabel({ label }) {
  return (
    Near.view("devhub.near", "get_posts_by_label", {
      label,
    }) ?? null
  );
}
