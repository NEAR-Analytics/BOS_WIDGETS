...
const ApyAndTvlWrapper = styled.div`
  font-size: 18px;
  font-weight: 600;
...
function updateInputValue(value) {
  if (!isSignedIn) {
    State.update({
      unstakeAmountInputError: "Sign in please",
    });
    return;
  }
  if (
    ...
    State.update({
      unstakeAmount: removePrefix0(value),
      unstakeAmountInputError: undefined,
    });
  }
  ...
  if (
    isDigit(value) &&
    Big(value).gt(
      Big(state.stakedNeatAmountRaw ?? 0).div(Big(10).pow(config.neatDecimals))
    )
  ) {
    State.update({
      unstakeAmountInputError: "Insufficient balance",
    });
  }
}
...
function processUnstake() {
  const tx = {
    ...
    methodName: "unstake",
    args: {
      amount: Big(state.unstakeAmount)
        .times(Big(10).pow(config.neatDecimals))
        .toFixed(0),
    },
    gas: TGas.times(300), // 300 TGas
    deposit: "1", // 1 yocto near
  };
  Near.asyncView(config.stakingContractName, "get_unpaid_storage_balance_of", {
    user_id: accountId,
  }).then((storageBalance) => {
    if (Big(storageBalance).gt(0)) {
      Near.call([
        {
          contractName: config.stakingContractName,
          methodName: "storage_deposit",
          deposit: storageBalance,
        },
        tx,
      ]);
    } else {
...
if (!state.startFetchReleasedAmountInterval) {
  State.update({ startFetchReleasedAmountInterval: true });
  setInterval(() => {
    fetchReleasedAmount();
  }, 1000);
}
  ...
  <>
    <Widget
      ...
      props={{
        ...
        maxTitle: "Available Balance: ",
        maxValue: state.stakedNeatAmount,
        unit: "$NEAT",
        onChange: updateInputValue,
        onClickMax: () =>
          updateInputValue(
            Big(state.stakedNeatAmountRaw ?? 0)
              .div(Big(10).pow(config.neatDecimals))
              .toFixed()
          ),
        error: state.unstakeAmountInputError,
      }}
    />
    <FormButton
      disabled={
        !!state.unstakeAmountInputError ||
        !isDigit(state.unstakeAmount) ||
        Big(state.unstakeAmount).lte(0)
      }
      onClick={() => {
        const pendingUnstake = Big(state.userUnstakedSeedRaw).sub(
          state.releasedAmountRaw
        );
        if (
          pendingUnstake.eq(0) && // pending unstake eq 0
          Big(state.releasedAmountRaw).gt(0)
        ) {
          State.update({ showWithdrawModal: true });
          return;
        } else if (pendingUnstake.gt(0)) {
          State.update({ showStillUnstake: true });
          return;
        }
        processUnstake();
      }}
    ...
    <OrangeText>
      *** It takes 21 days before all unstaked amount can be withdrawn ***
    </OrangeText>
    {/* still unstake */}
    {state.showStillUnstake && (
      <AbsoluteContainer>
        <FormContainer>
          <FormBody>
            You have{" "}
            {state.userUnstakedSeedRaw && state.releasedAmountRaw
              ...
              : "-"}{" "}
            $NEAT in pending unstake status. New unstake will reset the waiting
            cycle to 21 days.
          </FormBody>
          <WithdrawRow>
            <OutlineButton
              onClick={() => {
                State.update({
                  showStillUnstake: false,
                });
              }}
            >
              Cancel
            </OutlineButton>
            <OutlineButton onClick={processUnstake}>
              Still Unstake
            </OutlineButton>
          </WithdrawRow>
        </FormContainer>
      ...
      <AbsoluteContainer>
        <FormContainer style={{ maxWidth: "500px" }}>
          ...
          <WithdrawRow>
            ...
            <div>{state.releasedAmount} $NEAT</div>
          </WithdrawRow>
          <WithdrawRow>
            <OutlineButton
              onClick={() => {
            ...
            <OutlineButton onClick={onClickWithdraw}>Withdraw</OutlineButton>
          </WithdrawRow>
        </FormContainer>
      </AbsoluteContainer>
    )}
