...
function updateInputValue(value) {
  if (!isSignedIn) {
    State.update({
      stakeAmountInputError: "Sign in please",
    });
    return;
  }
  if (
    ...
    State.update({
      stakeAmount: removePrefix0(value),
      stakeAmountInputError: undefined,
    });
  }
  ...
  if (isDigit(value)) {
    if (
      Big(value).gt(
        Big(state.wrappedFtBalance ?? 0).div(Big(10).pow(config.neatDecimals))
      )
    ) {
      State.update({
        stakeAmountInputError: "Insufficient balance",
      });
    }

    if (Big(value).lt(0.1)) {
      State.update({
        stakeAmountInputError: "The stake amount must be greater than 0.1",
      });
    }
  }
}
  ...
  <>
    <Widget
      ...
      props={{
        ...
        error: state.stakeAmountInputError,
      }}
    />
    <FormButton
      disabled={
        !!state.stakeAmountInputError ||
        !isDigit(state.stakeAmount) ||
        Big(state.stakeAmount).lte(0)
      }
      onClick={() => {
        const tx = {
          ...
          args: {
            receiver_id: config.stakingContractName,
            amount: Big(state.stakeAmount)
