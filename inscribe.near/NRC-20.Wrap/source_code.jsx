...
function updateInputValue(value) {
  ...
  if (
    isDigit(value) &&
    Big(value).gt(
      Big(state.nrc20BalanceRaw ?? 0).div(Big(10).pow(state.decimals ?? 0))
    )
  ) {
    State.update({
...
function updateSelectValue(ticker) {
  ...
  const registered = state.registeredTokenContracts
    ?.map((contract) => contract.split(".")[0])
    ?.includes(tickerBalance.ticker.toLowerCase());
  if (!registered) {
    State.update({
      nep141Balance: "0",
      nep141BalanceError: "The ticker hasn't registered NEP-141 wrapper token",
    });
  } else {
    const contract = state.registeredTokenContracts.find(
      (contract) =>
        contract.split(".")[0] === tickerBalance.ticker.toLowerCase()
    );
    getNep141Balance(contract).then((balance) => {
      State.update({
        nep141Balance: formatAmount(balance, tokenInfo.decimals),
        nep141BalanceError: undefined,
      });
    });

    getWrapFeeRate(contract)
      .then((fee) => {
        State.update({
          wrapFee: fee,
        });
  ...
  if (tokenInfo?.decimals) {
    State.update({
      nrc20BalanceRaw: tickerBalance.amount,
  ...
  <>
        ...
        props={{
          ...
          data:
            state.balances && state.tokenInfos && state.registeredTokenContracts
              ? state.registeredTokenContracts
                  .map((contract) => {
                    const ticker = contract.split(".")[0].toLowerCase();
                    const balance = state.balances.find(
                      (balance) => balance.ticker.toLowerCase() === ticker
                    );

                    if (balance) {
                      const tokenInfo = state.tokenInfos.find(
                        (tokenInfo) => tokenInfo.ticker.toLowerCase() === ticker
                      );
                      if (tokenInfo?.decimals) {
                        return {
                          ticker: ticker,
    ...
    <Widget
      ...
      props={{
        ...
        unit: state.unit ? `$${state.unit.toUpperCase()}` : "",
        variant: "grey",
        error: state.nep141BalanceError,
        value:
