...
function getCountdownTime() {
  if (!config.firstFarmStartTimeUTC) {
    return undefined;
  }
  const d = new Date(config.firstFarmStartTimeUTC);
  const diff = d.getTime() - Date.now();
  if (diff <= 0) {
    return null;
  }
  const day = Math.floor(diff / DAY_OF_MS);
  const hour = Math.floor((diff - DAY_OF_MS * day) / HOUR_OF_MS);
  const min = Math.floor(
    (diff - day * DAY_OF_MS - hour * HOUR_OF_MS) / MIN_OF_MS
  );
...
if (!state.startFetchUserRewardsInterval) {
  State.update({ startFetchUserRewardsInterval: true });
  setInterval(() => {
    fetchUserRewards();
  }, 1000);
}

function getFirstFarmStartTimeUTCText() {
  const date = new Date(config.firstFarmStartTimeUTC);
...
return (
  <FormContainer style={{ fontWeight: "bold" }}>
    <InfoOuterWrapper>
      <div style={{ flex: 1 }}>
        <InfoRow>
          <div>Your Staked $NEAT</div>
          <div>{state.stakedNeatAmount} $NEAT</div>
        </InfoRow>
        <InfoRow>
          <div>Claimable $NEAR</div>
          <div>{state.claimableRewards} $NEAR</div>
          <OutlineButton
            disabled={
              !state.userRewardsPositive ||
              state.userRewardsPositive.length === 0
            }
            onClick={() => {
              const accountId = props.accountId || context.accountId;
              const allRewardsAmount = state.userRewardsPositive.reduce(
                (prev, cur) => {
                  return prev.add(cur.amount);
                },
                Big(0)
              ...
              Near.asyncView(
                config.stakingContractName,
                "get_unpaid_storage_balance_of",
                {
                  user_id: accountId,
                }
              ).then((unPaidStorageBalance) => {
                Near.asyncView(config.wNearTokenId, "storage_balance_of", {
                  account_id: accountId,
                }).then((tokenStoragebalance) => {
                  let txs = [
                    {
                    ...
                    ...state.userRewardsPositive.map((userReward) => ({
                      contractName: config.stakingContractName,
                      methodName: "claim_rewards",
                      args: {
                        farm_id: userReward.farmId,
                      },
                      gas: TGas.mul(50),
                    })),
                    {
                      contractName: config.wNearTokenId,
                      methodName: "near_withdraw",
                      args: {
                        amount: allRewardsAmount.toFixed(0),
                      },
                      deposit: "1", // 1 yocto near
                    },
                  ];

                  if (!tokenStoragebalance) {
                    txs = [
                      {
                        contractName: config.wNearTokenId,
                        methodName: "storage_deposit",
                        deposit: Big(0.00125)
                          .times(Big(10).pow(config.nearDecimals))
                          .toFixed(0),
                      },
                      ...txs,
                    ];
                  ...
                  if (Big(unPaidStorageBalance).gt(0)) {
                    txs = [
                      {
                        contractName: config.stakingContractName,
                        methodName: "storage_deposit",
                        deposit: unPaidStorageBalance,
                      },
                      ...txs,
                    ];
                  }
            ...
            <InfoRow>
              <div>
                <div style={{ marginBottom: "8px" }}>
                  Farming Starts in: {state.countdownTime}
                </div>
                <OrangeText>
                  $NEAR rewards farming will kick off on{" "}
                  {getFirstFarmStartTimeUTCText()}, UTC-0
                </OrangeText>
              </div>
            </InfoRow>
          </>
