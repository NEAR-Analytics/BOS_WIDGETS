...
if (props.transactionHashes) {
  switch (props.tab) {
    case CART_TAB:
      const { clearCart } = VM.require("old.potlock.near/widget/SDK.cart") ?? {
        clearCart: () => {},
      };
...
const tabContent = (
  <Widget
    src={`old.potlock.near/widget/${getTabWidget(props.tab)}`}
    props={props}
  />
);
...
return (
  <Theme>
    <Widget src={"old.potlock.near/widget/Components.Nav"} props={props} />
    <Content className={isForm ? "form" : ""}>{tabContent}</Content>
    <Widget src={"old.potlock.near/widget/Components.Attribution"} />
    {props.tab !== POT_DETAIL_TAB && props.tab !== POTS_TAB && (
      <Widget src={"old.potlock.near/widget/Components.Banner"} props={props} />
    )}
    <Widget
