      ...
      while (curIndex < allDonors.size) {
        promises.push(
          Near.asyncView("v1.nadabot.near", "get_human_score_batch", {
            account_ids: Array.from(allDonors).slice(
              curIndex,
              curIndex + limit
            ),
          })
      ...
      Promise.all(promises)
        ...
        .finally(() => {
          ...
          for (const [proj, user, amount] of projectContributions) {
            ...
            contributions[proj][user] = Big(
              contributions[proj][user] || 0
            ).plus(amount);
          }
          ...
          for (const contribz of Object.values(contributions)) {
            for (const [k1, v1] of Object.entries(contribz)) {
              ...
              for (const [k2, v2] of Object.entries(contribz)) {
                ...
                pairTotals[k1][k2] = pairTotals[k1][k2].plus(
                  v1.times(v2).sqrt()
                );
              }
            }
          ...
          if (residual.abs().gt(Big("0"))) {
            ...
            if (residual.abs().gt(Big(0))) {
              ...
              if (residual.gt(Big(0))) {
                ...
                totals[0].matching_amount_str = Big(
                  totals[0].matching_amount_str
                )
                  .plus(residual)
                  .toFixed(0);
              } else {
                ...
                for (let i = 0; i < totals.length; i++) {
                  if (Big(totals[i].matching_amount_str).gt(residual.abs())) {
                    totals[i].matching_amount_str = Big(
                      totals[i].matching_amount_str
                    )
                      .plus(residual)
              ...
              residual = totalPot.minus(totalAllocatedBeforeRounding);
              console.log(
                "Residual after final adjustment: ",
                residual.toFixed(0)
              );
            }
          }
