...
const [successfulDonation, setSuccessfulDonation] = useState(
  props.successfulDonation
);
const [ftMetadata, setFtMetadata] = useState(null);
const { recipientProfile, successfulApplication } = state;
const successfulDonationVals = successfulDonation
  ? Object.values(successfulDonation)
  : [];
const recipientProfileVals = recipientProfile
  ? Object.values(recipientProfile)
  : [];
...
useEffect(() => {
  if (successfulDonation && !ftMetadata) {
    Near.asyncView(successfulDonationVals[0]?.ft_id, "ft_metadata", {}).then(
      (metaDate) => setFtMetadata(metaDate)
    );
  }
}, [successfulDonation]);
const twitterIntent = useMemo(() => {
  ...
  let url =
    ...
    (successfulDonationVals[0].potId
      ? "old.potlock.near/widget/Index?tab=pot&potId=${successfulDonationVals[0].potId}&referrerId=${context.accountId}"
      : "old.potlock.near/widget/Index?tab=project&projectId=${recipient_id}&referrerId=${context.accountId}");
      ...
      children: successfulApplication ? (
        <>
          <ModalMain>
            ...
            <TextBold>
              Your application status: {successfulApplication.status}
            </TextBold>
          </ModalMain>
        </>
      ) : successfulDonation ? (
        <ModalMain>
          ...
          />
          {needsToVerify && !successfulDonationVals[0]?.recipient_id && (
            <VerifyInfo />
          )}
        </ModalMain>
      ) : (
