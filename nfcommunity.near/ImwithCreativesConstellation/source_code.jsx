...
const data = Social.keys(`${accountId}/post/nqno`, "final", {
  return_type: "History",
});
...
const processData = (data) => {
  ...
  const allItems = accounts
    .map((account) => {
      ...
      const blockHeights = account[1].post.nqno;
...
return (
  <div>
    ...
    <CommitButton
      data={{
        post: { nqno: "I participated in the June 21 2023 Near Quiz Night" },
      }}
      onCommit={() => {
        State.update({
    ...
    <div>
      {allItems
        ? allItems.map(({ accountId }) => (
            <div>
              I participated in the June 21 2023 Near Quiz Night
              <Widget
                src="williamxx.near/widget/Attribution"
                props={{
                  dep: true,
