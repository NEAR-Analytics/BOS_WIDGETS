...
let people = [];
const peopleUrl = "#/scopalaffairs.near/widget/PeoplePageTrendingUsers";
...
State.init({
  currentPage: 0,
  isLoading: true,
  selectedTab: props.tab || "trending", // "everyone", in prod
  trendingUserData: [],
});
...
const getTrendingUsers = (page) => {
  try {
    if (page === 1) {
      ...
      const res = fetch(url);
      if (res.ok) {
        console.log("fetch", page, res.status);
      ...
      asyncFetch(url).then((res) => {
        if (res.ok) {
          console.log("async", page, res.status);
          const parsedResults = JSON.parse(res.body);
          updateTrendingUsers(parsedResults.data);
        } else {
          console.log("Error fetching data async.");
        }
      });
    }
  } catch (error) {
...
if (data) {
  ...
  Object.keys(data).forEach((accountId) => {
    ...
    if (
      ...
      (state.selectedTab === "trending" && isTrending)
    ) {
      result.push({
        accountId,
...
const showLoadMoreButton =
  !state.searchResults &&
  !state.trendingUserData &&
  people.length % limitPerPage === 0;
  ...
  <Wrapper className="container-xl">
    ...
    {!state.searchResults && (
      <Tabs>
        ...
        {context.accountId && ( // set to ! in prod
          <TabsButton
