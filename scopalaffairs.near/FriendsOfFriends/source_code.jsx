...
const updateState = (data, totalPages) => {
  State.update({
    isLoading: false,
  ...
  try {
    ...
    asyncFetch(url).then((res) => {
      if (res.ok) {
        ...
        // const totalPages = parsedResults.totalPages;
        updateState(parsedResults.data); //, totalPages);
      } else {
...
const loadMore = () => {
  console.log("clicked load more", totalPages);
