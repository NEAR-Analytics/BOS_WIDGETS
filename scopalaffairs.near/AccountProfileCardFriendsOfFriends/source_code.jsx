...
const ProfileListContainer = styled.div`
  ...
  //   left: 0;
  //   right: 0;
  //   height: 40px; /* Adjust the height of the faded area */
...
const Profile = styled.li`
  ...
  background-color: rgba(255, 255, 255, 0.8); /* Adjust the alpha value for fading */
  transition: background-color 0.3s ease; /* Smooth transition for fading */
`;
  ...
  <Card>
    ...
    {props.because_you_know && props.because_you_know.length > 0 && (
      <ProfileListContainer>
        Because you follow
        <ProfileList>
          {props.because_you_know.slice(0, 3).map((profile, index) => (
            <Profile key={index}>{profile}</Profile>
          ))}
          {props.because_you_know.length > 3 && (
            <span>{`+${props.because_you_know.length - 3} more`}</span>
          )}
        </ProfileList>
      </ProfileListContainer>
    )}
