...
const { href: linkHref } = VM.require("espen.near/widget/core.lib.url");

const { hasModerator } = VM.require(
  "espen.near/widget/core.adapter.devhub-contract"
);
...
const ProfileIcon = () => {
  ...
  return (
    ...
    >
      <Widget
        src="espen.near/widget/devhub.components.molecule.ProfileCard"
  ...
  <Navbar className="position-relative">
    ...
    <div className="d-flex gap-3 align-items-center">
      <LinksContainer>
        {links.map((link) => (
          <Widget
            src="espen.near/widget/devhub.components.molecule.NavbarDropdown"
            props={{
              title: link.title,
              href: link.href,
              links: link.links,
          ...
          {links.map((link, idx) =>
            link.href ? (
              <MobileLink
                ...
                href={`/espen.near/widget/app?page=${link.href}`}
              >
                {link.title}
              </MobileLink>
            ) : (
              link.links.map((it, idx) =>
                ...
                it.href.startsWith("https://") ? (
                  ...
                  <MobileLink
                    key={`nested-link-${idx}`}
                    className={link.href === props.page && "active"}
                    href={`/espen.near/widget/app?page=${it.href}`}
                  >
                    /{it.title}
                  </MobileLink>
                )
