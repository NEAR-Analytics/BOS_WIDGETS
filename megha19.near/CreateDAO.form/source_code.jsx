...
const initialFormState = typeToEmptyData(types["megha19.near/type/dao"]);
...
const handleStepComplete = (value) => {
  ...
  Object.keys(value).forEach((key) => {
    const properties = types["megha19.near/type/dao"].properties.find(
      (p) => p.name === key
    );
...
return (
  <>
    ...
    />
    <Widget
      src={`megha19.near/widget/CreateDAO.Step${state.step + 1}`}
      props={{
        ...
        renderFooter: (stepState, otherProps) => (
          <Widget
            src={`megha19.near/widget/CreateDAO.Footer`}
            props={{
