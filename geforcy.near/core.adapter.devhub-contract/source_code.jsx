function getRootMembers() {
  return Near.view("", "get_root_members") ?? null;
}
...
function hasModerator({ account_id }) {
  ...
  return (
    Near.view("", "has_moderator", { account_id }) ??
    null
  );
}

function createCommunity({ inputs }) {
  return Near.call("", "create_community", { inputs });
}

function getCommunity({ handle }) {
  return (
    Near.view("", "get_community", { handle }) ?? null
  );
}

function getFeaturedCommunities() {
  return (
    Near.view("", "get_featured_communities") ?? null
  );
}

function getAccountCommunityPermissions({ account_id, community_handle }) {
  return (
    Near.view("", "get_account_community_permissions", {
      account_id,
      community_handle,
    }) ?? null
  );
...
function updateCommunity({ handle, community }) {
  return Near.call("", "update_community", {
    handle,
    community,
  });
}
...
function deleteCommunity({ handle }) {
  return Near.call("", "delete_community", { handle });
}

function updateCommunityBoard({ handle, board }) {
  return Near.call("", "update_community_board", {
    handle,
    board,
  });
}
...
function updateCommunityGithub({ handle, github }) {
  return Near.call("", "update_community_github", {
    handle,
    github,
  });
}
...
function setCommunityAddons({ handle, addons }) {
  return Near.call("", "set_community_addons", {
    handle,
    addons,
  });
}
...
function setCommunityAddon({ handle, addon }) {
  return Near.call("", "set_community_addon", {
    handle,
...
function getAllAddons() {
  return Near.view("", "get_all_addons") ?? null;
}

function getAccessControlInfo() {
  return (
    Near.view("", "get_access_control_info") ?? null
  );
}

function getAllAuthors() {
  return Near.view("", "get_all_authors") ?? null;
}

function getAllCommunitiesMetadata() {
  return (
    Near.view("", "get_all_communities_metadata") ?? null
  );
}
...
function getAvailableAddons() {
  ...
  // return Near.view("", "get_available_addons") ?? null;
}

function getCommunityAddons({ handle }) {
  return Near.view("", "get_community_addons", {
    handle,
  });
}

function getCommunityAddonConfigs({ handle }) {
  return Near.view("", "get_community_addon_configs", {
    handle,
  });
}

function getAllLabels() {
  return Near.view("", "get_all_labels") ?? null;
}

function getPost({ post_id }) {
  return Near.view("", "get_post", { post_id }) ?? null;
}

function getPostsByAuthor({ author }) {
  return (
    Near.view("", "get_posts_by_author", { author }) ??
    null
  );
}
