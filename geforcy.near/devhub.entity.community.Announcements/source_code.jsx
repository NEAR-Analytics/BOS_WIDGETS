...
const [submittedAnnouncementData, setSubmittedAnnouncementData] =
  useState(null);
const communityAccountId = `${handle}.community.thomasguntenaar.near`;
      ...
      <MainContent className="max-width-100">
        <div className="d-flex flex-column gap-4">
          {context.accountId &&
            (communityData?.admins ?? []).includes(context.accountId) && (
              <div className="card p-4">
                <Widget
                  src={"geforcy.near/widget/devhub.entity.community.Compose"}
                  props={{
                    onSubmit: (v) => {
                      setSubmittedAnnouncementData(v);
                      setCommunitySocialDB({ handle, data: v });
                    },
                    profileAccountId: `${handle}.community.thomasguntenaar.near`,
          ...
          <div className={postsExists && "card p-4"}>
            <Widget
              src="geforcy.near/widget/devhub.components.organism.Feed"
              props={{
        ...
        <div className="d-flex flex-column gap-3">
          ...
          <div className="card p-4 d-flex flex-column gap-2">
            ...
            {(communityData?.admins ?? []).map((accountId) => (
              ...
              >
                <Widget
                  src="geforcy.near/widget/devhub.components.molecule.ProfileCard"
                  props={{ accountId }}
                />
              </div>
            ))}
