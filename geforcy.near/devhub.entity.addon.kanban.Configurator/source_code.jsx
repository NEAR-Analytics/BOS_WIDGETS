const Struct = VM.require("geforcy.near/widget/core.lib.struct");
...
const { uuid, withUUIDIndex } = VM.require(
  "geforcy.near/widget/core.lib.uuid"
);
...
const KanbanViewConfigurator = ({ handle, data, permissions, onSubmit }) => {
  const tags = useCache(
    () =>
      Near.asyncView("thomasguntenaar.near", "get_all_labels").then(
        (res) => res
      ),
    handle,
    {
              ...
              >
                <div className="d-flex flex-column gap-1 w-100">
                  ...
                  <div className="d-flex flex-column flex-1 align-items-start justify-content-evenly gap-1 p-2 flex-grow-1">
                    ...
                    <div className="w-100">
                      <Widget
                        src="geforcy.near/widget/devhub.feature.post-search.by-tag"
                        props={{
                          tag: tag,
  ...
  return (
    ...
    >
      ...
      {showPreview ? (
        <div>
          <Widget
            src={`geforcy.near/widget/devhub.entity.addon.kanban.Viewer`}
            props={{
              data: formState,
            }}
          />
