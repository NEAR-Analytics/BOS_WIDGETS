const widgetAccountId = "ostolex.near";
const host = "https://near.org";
...
const pages = {
  menu: "MENU",
  aiGame: "AI_GAME",
  pleaseRelogin: "RELOGIN",
  checkStorage: "CHECK_STORAGE",
...
// */laserchess3d/widgetAccountId-laserchess3d-room-120b1876-f31d-4198-87d2-f6b3fc6ba80e/
                ...
                <div class="container">
                  ...
                  <div class="row">
                    <div class="col-md-12" style="padding-top: 50px;">
                      <button type="button" id="go-home-button"><a href="${host}/${widgetAccountId}/widget/LaserChess3D" style="color: #fff; text-decoration: none;">Go to main menu</a></button>
                    </div>
                  </div>
                </div>
        </div>
...
const menuCallback = (isPve, sn) => {
  console.log(isPve ? pages.aiGame : pages.checkStorage);
  State.update({
    currentPage: isPve ? pages.aiGame : pages.pleaseRelogin,
    ...
    <Widget
      ...
      props={{
        ...
        connectToGameCallback: () =>
          State.update({
            currentPage: pages.pleaseRelogin,
...
if (state.currentPage == pages.pleaseRelogin) {
  return (
    <>
      <h2>
        Warning! The game might crush in case of signing transaction in wallet,
        so if you have less than 0.2 NEAR locked as prepaid gas, please relogin
        in your BOS gateway.
      </h2>
      <button
        class="btn btn-success"
        onClick={() => {
          State.update({ currentPage: pages.checkStorage });
        }}
      >
        {"I'm ready, go further!"}
      </button>
    </>
  );
}

const checkStorageCallback = () => {
  State.update({
    currentPage: pages.usersMatcher,
  });
};

if (state.currentPage == pages.checkStorage) {
  return (
    <Widget
...
if (state.currentPage == pages.usersMatcher) {
  return (
    <Widget
      src={`${widgetAccountId}/widget/UsersMatcher`}
      props={{
        widgetKey: "laserchess3d",
        widgetName: "Laser chess",
        loadRoomCallback: usersMatcherCallback,
        urlPrefix: `${host}/${widgetAccountId}/widget/LaserChess3D?roomId=`,
        initialValue: {
          sn: state.sn,
        },
      }}
  ...
  if (Object.keys(keysResult).length == 1) {
    return (
      <>
        ...
        <pre
          style={{ color: "red" }}
        >{`${host}/${widgetAccountId}/widget/LaserChess3D?roomId=${state.roomId}`}</pre>
      </>
    );
  }
...
if (state.currentPage == pages.game) {
  if (props.roomId) {
    return (
      <Widget
        src={`${widgetAccountId}/widget/LaserChess3DOnline`}
        props={{
          exitCallback: exit,
          roomId: state.roomId,
    ...
    return (
      <>
        <h1>Your room created</h1>
        <a
          href={`${host}/${widgetAccountId}/widget/LaserChess3D?roomId=${state.roomId}`}
          target="_blank"
        >
          Open the game
        </a>
