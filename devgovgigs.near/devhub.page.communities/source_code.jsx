const { getAllCommunitiesMetadata, createCommunity } = VM.require(
  "devgovgigs.near/widget/core.adapter.devhub-contract"
);
...
return (
  <div className="w-100">
    <Widget
      src={`devgovgigs.near/widget/devhub.components.island.banner`}
      props={{
        title: (
          <>
            We are building <br />
    ...
    <div style={{ background: "#f4f4f4" }}>
      ...
      >
        <div className="d-flex flex-column gap-3 w-100">
          <h1 className="m-0 fs-4">
            <Link
              to={"/devgovgigs.near/widget/app?page=communities"}
      ...
      <div className="d-flex flex-wrap align-content-start gap-4 p-4 w-100 h-100">
        {showSpawner && (
          <Widget
            src="devgovgigs.near/widget/devhub.entity.community.Spawner"
            props={{
              data: null,
        ...
        <CardGrid>
          {searchKey === "" && sort === ""
            ? (communitiesMetadata ?? []).reverse().map((communityMetadata) => (
                <Widget
                  src="devgovgigs.near/widget/devhub.entity.community.Card"
                  props={{
                    format: "small",
