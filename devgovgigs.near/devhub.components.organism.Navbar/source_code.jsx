...
const { href: linkHref } = VM.require("devgovgigs.near/widget/core.lib.url");
  ...
  <Navbar className="position-relative">
    ...
    <div className="d-flex gap-3 align-items-center">
      <LinksContainer>
        {links.map((link) => (
          <Widget
            src="devgovgigs.near/widget/devhub.components.molecule.NavbarDropdown"
            props={{
              title: link.title,
              href: link.href,
              links: link.links,
          ...
          {links.map((link, idx) =>
            link.href ? (
              <MobileLink
                ...
                href={`/devgovgigs.near/widget/app?page=${link.href}`}
              >
                {link.title}
              </MobileLink>
            ) : (
              link.links.map((it, idx) => (
                <MobileLink
                  key={`nested-link-${idx}`}
                  className={link.href === props.page && "active"}
                  href={`/devgovgigs.near/widget/app?page=${it.href}`}
                >
                  {it.title}
                </MobileLink>
              ))
