  ...
  const data = {
    index: {
      star: JSON.stringify({
        key: item,
        value: {
          type: hasStar ? "unstar" : "star",
        },
      }),
    },
  };

  // Check if it's a widget, then we'll add it to a graph
  // Temporary remove it. Only add to the index.
  /*
  if (item.type === "social" && typeof item.path === "string") {
    const keys = item.path.split("/");
    if (keys.length === 3 && keys[0] && keys[1] === "widget" && keys[2]) {
      data.graph = {
        star: {
          [keys[0]]: {
            [keys[1]]: {
...
const title = hasStar ? "Unstar" : "Star";
