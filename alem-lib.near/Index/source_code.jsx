  ...
  /** Bundle generated by Além Library v1.0.0-beta.8 - See more here: https://github.com/wpdas/alem */
  /** Project repository: https://github.com/wpdas/alem-docs-web */
  const updateAlemState = (updatedState) => {
  State.update({
    alem: {
      ...state.alem,
      ...updatedState,
...
const props = {
  ...
  alem: {
    ...state.alem,
    createRoute: (path, component) => ({ path, component }) ,
    useParams: () => {
  ...
  useEffect(() => {
    ...
    const maintainRoutesWhenDeveloping = alem.isDevelopment && alem.alemConfig_maintainRouteWhenDeveloping;
    if (routes) {
      let currentUrlPath = bosProps[routeParamName] && checkIfPathIsIncludedToRoutes(bosProps[routeParamName]) ? bosProps[routeParamName] : routes[0].path;
      const _routes = routes.map(route => route.path);
      const _type = type || "URLBased";
      let _activeRoute = initialRoute || alemRoutes.activeRoute || currentUrlPath;
      if (!(currentUrlPath && routeType == "URLBased" && alemRoutes.routeBlocked)) {
        _activeRoute = maintainRoutesWhenDeveloping ? initialRoute || activeRoute : routes[0].path;
      }
  ...
  const Component = routes.find(route => route.path === activeRoute)?.component || routes[0].component || <></>;
  return <Component />;
    `,
        Sidebar: `
          const Categories = {
...
const RoutesPath = {
  ...
  const routeContext = useContext("alemRoutes");
  if (!routeContext) {
  ...
  const routeContext = useContext("alemRoutes");
  if (!routeContext) {
    console.error("RouteLink component is being used without Router on top of it.");
  }
  ...
  if (routeContext.routeType === "URLBased") {
    ...
    return <a onClick={onClickHandler} className={className} style={{
      cursor: "pointer",
      textDecoration: "none",
      ...style
  ...
  return <a style={{
    ...
    ...style
      ...
      {label || children}
          ...
          const getLinksByCategory = (category, onItemClick) => {
  const {
    pathname
  } = getLocation();
  return Object.keys(RoutesPath).map(routeKey => {
    const routeDetails = RoutesPath[routeKey];
    if (routeDetails.category === category) {
      return <RouteLink to={routeDetails.path} onClick={onItemClick} label={routeDetails.title} className={pathname === routeDetails.path ? "active" : ""} />;
    }
  }).filter(item => !!item);
};
          const A_6 = styled.div\`
  display: flex;
...
\`;
const SidebarAboutLogo = styled.img\`
  align-self: center;
  @media (max-width: 800px) {
    display: none;
  }
\`;
  ...
  return contextItems;
          ...
          const useRoutes = () => {
  const contextData = useContext("alemRoutes");
  if (!contextData) {
  ...
  const [gettingStartedItems, setGettingStartedItems] = useState([]);
  const [routerItems, setRouterItems] = useState([]);
  const [contextItems, setContextItems] = useState([]);
  const [apisItems, setApisItems] = useState([]);
  ...
  return <A_6>
      <SidebarAboutLogo src="https://cdn.jsdelivr.net/gh/wpdas/alem/docs/assets/near-script-logo.png" width={60} alt="logo" />
      <div>
...
\`;
  ...
  return <A_7>
      <NavBar>
        <RouteLink to="about">
          <A_8 src="https://cdn.jsdelivr.net/gh/wpdas/alem/docs/assets/near-script-logo.png" width={40} alt="logo" />
        </RouteLink>
        <CollapseButton type="button" onClick={() => {
        setShow(!show);
      }}>
          {!show ? <span className="material-symbols-outlined">menu</span> : <span className="material-symbols-outlined">close</span>}
        </CollapseButton>
      </NavBar>
...
\`;
          const GithubIcon = () => <svg _ngcontent-ng-c1539744558="" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 56 56">
    <g _ngcontent-ng-c1539744558="" fill-rule="evenodd" clip-rule="evenodd">
      ...
      <path _ngcontent-ng-c1539744558="" fill="#181616" d="M28 0C12.54 0 0 12.54 0 28c0 12.37 8.02 22.86 19.15 26.57 1.4.26 1.91-.61 1.91-1.35 0-.66-.02-2.43-.04-4.76-7.79 1.69-9.43-3.75-9.43-3.75-1.27-3.23-3.11-4.1-3.11-4.1-2.54-1.74.19-1.7.19-1.7 2.81.2 4.29 2.89 4.29 2.89 2.5 4.28 6.55 3.04 8.15 2.33.25-1.81.98-3.04 1.78-3.74-6.22-.71-12.75-3.11-12.75-13.84 0-3.06 1.09-5.56 2.88-7.51-.29-.71-1.25-3.56.27-7.41 0 0 2.35-.75 7.7 2.87 2.23-.62 4.63-.93 7.01-.94 2.38.01 4.77.32 7.01.94 5.35-3.62 7.69-2.87 7.69-2.87 1.53 3.85.57 6.7.28 7.41 1.79 1.96 2.88 4.46 2.88 7.51 0 10.76-6.55 13.12-12.78 13.82 1.01.86 1.9 2.57 1.9 5.19 0 3.74-.03 6.76-.03 7.68 0 .75.5 1.62 1.93 1.35C47.98 50.86 56 40.37 56 28 56 12.54 43.46 0 28 0z"></path>
    </g>
  </svg>;
          const TelegramIcon = () => <svg width="24px" height="24px" version="1.1" xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink" xmlSpace="preserve" style={{
  fillRule: "evenodd",
  clipRule: "evenodd",
  ...
  return <Container>
      <Logo>
        <img src="https://cdn.jsdelivr.net/gh/wpdas/alem/docs/assets/near-script-logo.png" width={30} alt="logo" />
        <p>Além</p>
      </Logo>
      <Row>
        <Description>
...
const Margin = styled.div\`\`;
  ...
  display: flex;
  justify-content: \${props => props.hasPrevious && props.hasPrevious ? "space-between" : !props.hasPrevious && props.hasNext ? "flex-end" : props.hasPrevious && !props.hasNext ? "flex-start" : "flex-end"};
  margin-top: 28px;
\`;
...
\`;
  ...
  return <Markdown text={mdContent} />;
          ...
          const Cli = () => {
  const mdContent = Buffer.from(\`\`, "base64").toString("utf-8");
  return <Markdown text={mdContent} />;
};
          const ConfigFile = () => {
  const mdContent = Buffer.from(\`\`, "base64").toString("utf-8");
  return <Markdown text={mdContent} />;
};
          const Installation = () => {
...
\`;
  ...
  return <div>
      ...
      marginBottom: 0
        ...
        <A_3 src="https://cdn.jsdelivr.net/gh/wpdas/alem/docs/assets/near-script-logo.png" width={120} alt="logo" />
        <A_4>Além</A_4>
        <A_5>The library to build web3 applications for Near BOS</A_5>
        <Row>
      ...
      paddingTop: 40
        ...
        <img src="https://cdn.jsdelivr.net/gh/wpdas/alem-docs-web/src/md/assets/diagram.jpg" width="100%" alt="Diagram" />
        <p>
      ...
      <Section>
        <A_4>Create user interfaces from components</A_4>
        <p>
      ...
      backgroundColor: "#fff"
        ...
        <A_4>Stateless & Stateful Component</A_4>
        <p>
...
const Routes = () => {
const urlParams = props.alem.useParams();
...
return <>
  ...
  return <A_1>
      {pathname !== "about" && pathname !== "" && <TopSection>
          <div>
...
\`;
const MobileNavBarSwitcher = styled.div\`
  font-family: "Wix Madefor Display", sans-serif;
  /* display: none; */
  height: 0px;
  overflow: hidden;
...
\`;
          const ShareIcon = () => <span className="material-symbols-outlined">share</span>;
          const SharedInfoModal = () => {
  return <>
      <div className="modal fade" id="sharedInfoModal" tabIndex={-1} aria-labelledby="exampleModalLabel" aria-hidden="true">
    ...
    <SharedInfoModal />
          ...
          const createContext = contextKey => {
  const setDefaultData = defaultStateValue => {
    if (!state[contextKey] || !state[contextKey].initialized) {
      const stateKeys = Object.keys(defaultStateValue);
      let mainKeys = [...stateKeys];
      mainKeys = mainKeys.filter((item, index) => mainKeys.indexOf(item) === index);
      State.update({
        ...state,
        [contextKey]: {
          initialized: true,
...
const RouterProvider = () => {
  const {
    setDefaultData,
    updateData
  } = createContext(ALEM_ROUTES_CONTEXT_KEY);
  const updateAlemRoutesState = updatedState => {
    updateData({
      ...updatedState
    });
  };
  const alemRoutesState = () => useContext(ALEM_ROUTES_CONTEXT_KEY);
  setDefaultData({
    ...
    routeType: "URLBased",
    routeBlocked: true,
    updateRouteParameterName: parameterName => {
...
RouterProvider();
return <div style={{
marginTop: "calc(-1 * var(--body-top-padding, 0))"
}}>
      <Modals />
      <Banner>
      ...
      <MobileNavBarSwitcher>
        <Widget loading=" " code={props.alem.componentsCode.MobileNavBar} props={{ ...{ ...props } }} />
      </MobileNavBarSwitcher>
      <AppContainer>
        <SideBarSwitcher>
          <Widget loading=" " code={props.alem.componentsCode.Sidebar} props={{ ...{ ...props } }} />
        </SideBarSwitcher>
        <ContentView />
      </AppContainer>
      <Footer />
...
const stylesLoaded = props.alem.loadExternalStyles(["https://fonts.googleapis.com/css2?family=Wix+Madefor+Display:wght@400..800&display=swap", "https://fonts.googleapis.com/css2?family=Material+Symbols+Outlined:opsz,wght,FILL,GRAD@24,400,0,0", "https://fonts.cdnfonts.com/css/source-code-pro"]);
if (!stylesLoaded) {
return <Loading />;
}
return <Widget loading=" " code={props.alem.componentsCode.Main} props={{ ...{ ...props } }} />;
          `,
    },
  },
};
...
}const alemCssBody = ``;const AlemTheme = styled.div`
