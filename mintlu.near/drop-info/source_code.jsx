...
// function handleTicket(key_id) {
//   console.log(`Here and the key_id is: ${key_id}`);
//   var drop_id = my_drops[selectedDropIndex].drop_id;
//   var numKeys = (key_id + 1).toString();
//   asyncFetch(
...
//     let pub = keyPairs[key_id].pub;
//     let priv = keyPairs[key_id].priv;
//     var viewTicket = `https://test.near.social/#/minqi.testnet/widget/kp-ticket-homepage?tab=misc&key=${pub}&pk=${priv}`;
...
function getTicketLinks() {
  ...
  asyncFetch(
    ...
    keyPairs.forEach((e) => {
      allTickets.push(
        `https://near.org/mintlu.near/widget/kp-ticket2?key=${e.pub}&pk=${e.priv}`
      );
    });
    State.update({
      allTicketLinks: allTickets,
    });
  });
}
        ...
        <div class="dropdown d-flex justify-content-between">
          ...
          <div className="counts-container d-flex justify-content-center align-items-center">
            ...
            <div className="count-item">
              ...
              >
                {state.count.used > 0 ? state.count.used : 0}
              </span>
            </div>
          </div>
        </div>
          ...
          <tbody>
            {state.keys.map((key) => (
              <tr key={key.key_id}>
                <td>{key.key_id}</td>
                <td>
                  <a href={state.allTicketLinks[key.key_id]}>{key.pk}</a>
                </td>
                <td>
                  {key.remaining_uses ===
