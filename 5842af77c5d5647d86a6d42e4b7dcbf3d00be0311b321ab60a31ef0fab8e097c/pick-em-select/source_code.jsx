...
State.init({
  selections: [],
  matches: Near.view("neetion.near", "getMatches").filter(
    (m) => new Date() < new Date(m.time)
  ),
  predictions: Near.view("neetion.near", "getAllPicks"),
});

const onTeamSelect = (team, matchIndex) => {
  let _selections = state.selections.filter((t) => t.index !== matchIndex);
  _selections.push({
    predictor: context.accountId,
    team: team,
    index: matchIndex,
  });
...
const onSubmit = () => {
  return Near.call("neetion.near", "bulkPick", { picks: state.selections });
};
...
const button = (name, image, first, selected, onClick) => (
  <button
    ...
    style={{
      ...
      flexDirection: first ? "row-reverse" : "reverse",
...
const matchup = (index, team1, team2, isActive) => (
  ...
  >
    {button(
      ...
      team1.image,
      true,
      state.selections.some((s) => s.index === index && s.team === 1),
    ...
    {button(
      ...
      team2.image,
      false,
      state.selections.some((s) => s.index === index && s.team === 2),
    ...
    >
      ...
      >
        {context.accountId ? (
          ...
          >
            Signed in as {context.accountId}
          </div>
        ) : (
          <div
            style={{
