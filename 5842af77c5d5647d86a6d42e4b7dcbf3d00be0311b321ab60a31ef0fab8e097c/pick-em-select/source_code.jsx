...
State.update({
  matches: Near.view("pickem.near", "getMatches").filter(
    (m) => new Date() < new Date(m.time)
  ),
  predictions: Near.view("pickem.near", "getAllPicks"),
});
...
const onSubmit = () => {
  Near.call("pickem.near", "bulkPick", state.selections).then(() => {
    let _picks = Near.view("pickem.near", "getAllPicks");
    console.log("_picks", _picks);
  });
  let _picks2 = Near.view("pickem.near", "getAllPicks");
  console.log("_picks2", _picks2);
};
