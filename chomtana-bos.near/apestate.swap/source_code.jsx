...
State.init({
  ...
  hoverOnChain: "",
  estimate: {},
  inputAssetTokenId: "0x8FDcf87434723D9392Cb07434ef76a578a080c16",
  outputAssetTokenId: "0x0000000000000000000000000000000000000001",
  coinGeckoTokenIds: {
  ...
  noPool: false,
  longShort: "long",
  routerContract: "0x02a28267A27545D2c4B5BB700d04319117154959",
  mul: 1,
  positions: [],
});
...
if (state.landAbi === undefined) {
  const landAbi = fetch(
    "https://gist.githubusercontent.com/Chomtana/8c3e04956fce0ab2e1c2de10235278ba/raw/40de80660fa9fd07c7ab85cc78f9088078021978/LandExchangeAbi.json"
  );
  if (!landAbi.ok) {
    return "Loading";
  }
...
const changeAmount = (e) => {
  ...
  let metadata = tokenMetas[state.outputAssetTokenId];

  console.log(amountFloat, metadata);

  if (!isNaN(amountFloat) && metadata && metadata.price) {
    State.update({
      inputAssetAmount: amount,
      outputAssetAmount: (amountFloat / metadata.price / THBUSD).toFixed(4),
    });
  } else {
...
const callTokenApprovalEVM = (input, onComplete, gweiPrice, gasLimit) => {
  if (
    ...
    if (gweiPrice !== undefined && gasLimit !== undefined) {
      gasArgs.gasLimit = gasLimit ?? 2000000;
    }
...
const callTxApestateLong = (
  ...
  if (
    ...
    const price = tokenMetas[input.outputAssetTokenId].price;

    const assetValue = Big(value)
      .div(price * THBUSD)
      .mul(0.995)
      .round()
      .toString();

    console.log("input.inputAsset: ", input.outputAssetTokenId, value);
    ...
    routerContract
      .buy(input.outputAssetTokenId, value, gasArgs)
      .then((transactionHash) => {
        transactionHash.wait().then(() => {
          const positions = [...state.positions];
          positions.push({
            type: "long",
            landId: input.outputAssetTokenId,
            usdValue: value,
            tokenValue: assetValue,
          });
...
const callTxApestateShort = (
  ...
  if (
    ...
    routerContract
      .borrowAndSell(input.outputAssetTokenId, value, assetValue, gasArgs)
      .then((transactionHash) => {
        transactionHash.wait().then((receipt) => {
...
const callTxApestateSell = (
  ...
  if (input.sender && input.erc20Abi && input.landAbi && input.routerContract) {
    const routerContract = new ethers.Contract(
      input.routerContract,
      input.landAbi,
      Ethers.provider().getSigner()
    ...
    routerContract
      .sell(tokenId, assetValue, gasArgs)
      .then((transactionHash) => {
        transactionHash.wait().then(() => {
          const positions = [...state.positions];
  ...
  if (
    ...
    return Ethers.provider()
      ...
      .then((rawBalance) => {
        ...
        State.update({
          outputAssetBalanceHr: parseFloat(
            ethers.utils.formatEther(receiverBalanceHex.toString())
          ).toFixed(4),
        });
      });
  }
      ...
      >
        <div class="swap-currency-input">
          <div className="swap-direction">
            {amountName === "inputAssetAmount" ? "From" : "To"}
          </div>
  ...
  if (
    ...
    return Ethers.provider()
      ...
      .then((encodedTokenAllowanceHex) => {
        ...
        console.log(
          "tokenAllowance: ",
          Big(tokenAllowance).div(Big(10).pow(18)).toFixed(),
...
const networkList = NETWORKS.map((network) => network.chainId); //  [1, 25925];
...
if (forceNetwork && state.network && forceNetwork !== state.network) {
  return (
    ...
    >
      To proceed, kindly switch to Bitkub Testnet.
      <Widget
        src="guessme.near/widget/ZKEVMSwap.zkevm-connect"
        props={{
          title: "Polygon zkEVM Swap",
          src: "https://apestate.vercel.app/_next/image?url=%2F_next%2Fstatic%2Fmedia%2Fapelogo2-01.6418a9d4.png&w=256&q=75",
          imgStyle: {
            width: "528px",
            // height: "216px",
          },
...
if (params && selectedChainId === 25925 && state.hasGetStorage === false) {
...
return (
  <>
    ...
    {state.network && state.inputAssetTokenId && (
      <Widget
        src="02f1a57368e5707a8cfb2744f72f416943d2fe2a4431ed382ba41f477f8fdec1/widget/apestate.asset-list"
        props={{
          network: state.network,
          assets: [state.inputAssetTokenId],
    ...
    >
      {false && state.network && state.dexName && !oneProtocol && (
        <div
          className=""
          style={{
            maxWidth: "100%",
      ...
      <div>
        <SwapPage>
          <div class="long-short-btn-container">
            <div
              className={
                "long-short-btn " + (state.longShort == "long" ? "active" : "")
              }
              onClick={() => State.update({ longShort: "long" })}
            >
              Long
            </div>
            ...
            <div
              className={
                "long-short-btn " + (state.longShort == "short" ? "active" : "")
              }
              onClick={() => State.update({ longShort: "short" })}
            >
              Short
            </div>
          </div>
          ...
          <div class="top-container">
            {assetContainer(
              ...
              // state.inputAsset,
              {
                ...
                readonly: true,
                balance_hr_full:
                  state.inputAsset.balance_hr_full &&
                  parseFloat(
                    ethers.utils.formatEther(
                      state.inputAsset.balance_hr_full.replace(".", "")
                    )
                  ).toFixed(2),
              },
              "inputAssetAmount",
          ...
          <div class="bottom-container">
            {assetContainer(
              ...
              {
                ...state.outputAsset,
                metadata: tokenMetas[state.outputAssetTokenId] || {},
                balance_hr_full: state.outputAssetBalanceHr,
              },
              "outputAssetAmount",
              () => {
            ...
            <div className="details-container">
              <div className="details-item">
                ...
                <div>
                  {Math.round(
                    tokenMetas[state.outputAssetTokenId].price * THBUSD
            ...
            <div className="mul-container">
              {MULS.map((mul) => (
                <div
                  className={"mul-item " + (state.mul == mul ? "active" : "")}
                  onClick={() => State.update({ mul })}
                  key={mul}
                >
                  {mul}x
                </div>
        ...
        {state.positions.length > 0 ? (
          <div>
            {state.positions.map((position, i) => (
              <div
                className={"position-item position-item-" + position.type}
                key={i}
              >
                <div>
                  ...
                  {": "}
                  {parseFloat(
                    ethers.utils.formatEther(position.tokenValue)
                  ).toFixed(4)}{" "}
                  {tokenMetas[position.landId].symbol}{" "}
                  {"(" +
                    parseFloat(
                      ethers.utils.formatEther(position.usdValue)
                    ).toFixed(2) +
                    " KUSDT)"}
                </div>

                <div
                  className="position-item-action"
                  onClick={() => {
          ...
          <div style={{ textAlign: "center", color: "rgb(209, 213, 219)" }}>
            --- No Open Positions ---
          </div>
        )}
      </SwapPage>
